...
contract Voting is Ownable{
	...
	mapping(uint => Proposal[]) private listProposals;
	...
	function setProposal(uint _index,string memory _description) external inProposalsRegistrationStatus(_index) anAuthorizedVoter(_index){
		...
		listProposals[_index].push(Proposal({
	 ...
	 */
	function getAllProposal(uint _index) external view returns(Proposal[] memory proposals_){
	 ...
	 */
	function setVoted(uint _index, uint _proposalId) external anAuthorizedVoter(_index) {
	...
	function tallyVotes(uint _index) external onlyOwner () {
       require(listWorkflowStatus[_index] == WorkflowStatus.VotingSessionEnded, "Current status is not voting session ended");
        ...
        uint nbWinners;
        for (uint i = 0; i < listProposals[_index].length; i++) {
	 ...
	 */
	function getWinningProposalsID(uint _index) external view returns(uint[] memory winningProposalsID_) {
	 ...
	 */
	function getWinningProposals(uint _index) external view returns(Proposal[] memory winningProposals_) {
