	...
	describe('action', () => {
		...
		describe('Add Proposal', () => {
			...
			it('should store a new voting session',async () =>{
				...
				proposal = await VotingInstance.setProposal(new BN(0),'Proposal One', {from: account.firstVoter});
				await VotingInstance.setProposal(new BN(0),'Proposal Two', {from: account.secondVoter});
			...
			for (let i: number = 0; i < 6; i++) {
				if (i < 4){
					...
					it('should give a tally of the votes', async () => {
						// expectRevert(VotingInstance.tallyVotes(0,{from: account.owner}), 'Cancel Session');
